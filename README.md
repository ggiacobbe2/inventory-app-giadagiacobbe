**This app stands as a basic inventory app where users can view, add, update, and delete items in their inventory, all while syncing real-time.**

**How to Use**\n
VIEW: Select any of the loaded items in the inventory list. View details such as name, quantity, price, and category./n
ADD: Click the '+' icon in the bottom right corner, enter
UPDATE: Select any item, make edit(s), and click 'Update Item' to save.
DELETE: Select any item, and click 'Delete Item' to delete. Alternatively, click the Bulk Action icon in the top right corner to select multiple items to delete at once.

**Advanced Features Implemented**
1. Advanced Search & Filtering
   a. Search by name.
   b. Filter by category, low stock items, or low price items.
2. Bulk Operations
   a. Click on the Bulk Action icon in the top right corner to select multiple items at once to be deleted.
